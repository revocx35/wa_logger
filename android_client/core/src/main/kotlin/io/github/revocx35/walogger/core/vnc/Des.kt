...
/**
 * Single-block DES encryption (FIPS 46-3), only for the VNC authentication challenge. Implemented
 * here because not every Android security provider still exposes plain DES; verified against the
 * JDK's implementation in the tests.
 */
internal object Des {
    ...
    /** Bit 1 = most significant bit of the [inBits]-wide input. */
    private fun permute(input: Long, inBits: Int, table: IntArray): Long {
        var out = 0L
        for (pos in table) out = (out shl 1) or ((input ushr (inBits - pos)) and 1L)
        return out
    }

    private fun rotl28(v: Long, n: Int): Long = ((v shl n) or (v ushr (28 - n))) and 0xFFFFFFFL
    ...
    /** DES-ECB encryption of [data] (a multiple of 8 bytes) with an 8-byte [key]. */
    fun encryptEcb(key: ByteArray, data: ByteArray): ByteArray {
        require(key.size == 8 && data.size % 8 == 0)
        val pc1 = permute(toLong(key, 0), 64, PC1)
        var c = (pc1 ushr 28) and 0xFFFFFFFL
        var d = pc1 and 0xFFFFFFFL
        val subkeys = LongArray(16) { i ->
            c = rotl28(c, SHIFTS[i])
            d = rotl28(d, SHIFTS[i])
            permute((c shl 28) or d, 56, PC2)
        }
        val out = ByteArray(data.size)
        for (blk in data.indices step 8) {
            val ip = permute(toLong(data, blk), 64, IP)
            var l = (ip ushr 32) and 0xFFFFFFFFL
            var r = ip and 0xFFFFFFFFL
            for (round in 0 until 16) {
                val e = permute(r, 32, E) xor subkeys[round]
                var s = 0L
                for (j in 0 until 8) {
                    val six = ((e ushr (42 - 6 * j)) and 0x3F).toInt()
                    val row = ((six and 0x20) ushr 4) or (six and 1)
                    val col = (six ushr 1) and 0xF
                    s = (s shl 4) or S[j][row * 16 + col].toLong()
                }
                val next = l xor permute(s, 32, P)
                l = r
                r = next
            }
            val res = permute((r shl 32) or l, 64, FP)
            for (i in 0 until 8) out[blk + i] = (res ushr (56 - 8 * i)).toByte()
        }
        return out
    }
}
